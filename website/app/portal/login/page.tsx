...
import { clientChangePassword, clientSignInWithPassword, getClientEngagementData } from '@/app/actions';
...
export default function ClientPortalLogin() {
  ...
  if (step === 'checking' || step === 'entering') {
    return (
      <div className="min-h-screen bg-[#0a0f2c] text-[#f1f5f9] flex items-center justify-center p-6">
        <div className="text-center space-y-4 max-w-sm">
          ...
          <p className="text-lg font-medium">
            {step === 'entering' ? 'Password saved — opening your portal…' : 'Loading…'}
          </p>
          ...
          {step === 'sign-in' ? (
            ...
            <form onSubmit={handleCreatePassword} className="space-y-4">
              ...
              <button
                ...
                className="w-full px-5 py-3.5 bg-[#8f6f3d] hover:bg-[#b89a6e] text-black font-semibold text-base rounded-full transition-all disabled:opacity-60"
              >
                {isLoading ? 'Saving…' : 'Save Password & Enter Portal'}
              </button>
            </form>
          )}
        </div>
