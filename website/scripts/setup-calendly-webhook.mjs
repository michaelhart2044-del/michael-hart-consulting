...
async function main() {
  ...
  if (!me.ok) {
    ...
    console.error('- Token needs scopes: webhooks:read, webhooks:write, users:read');
    process.exit(1);
  }
  ...
  if (list.ok && list.json?.collection?.length) {
    ...
    const duplicate = list.json.collection.find(
      (s) => s.callback_url === CALLBACK_URL && s.state === 'active',
    );
    if (duplicate) {
      ...
      console.log('Delete this subscription in Calendly (or via API) and re-run to set a fresh signing_key.\n');
    }
  } else if (!list.ok) {
